...
const StageFrameNode = React.memo(({ item, commonProps, setShapeRef, updateShape, onAction }: any) => {
  ...
  const nameWidth = Math.max(80, frameName.length * 9); 
  const pillWidth = nameWidth + 30; 
    ...
    >
      ...
      >
        <Rect
          width={pillWidth}
          height={26}
          fill={item.isLocked ? "#fee2e2" : "#f1f5f9"} 
          cornerRadius={[6, 6, 0, 0]} 
          stroke={item.isLocked ? "#ef4444" : "#cbd5e1"}
          strokeWidth={1}
        />
        ...
        >
             ...
             <Path
                data={item.isLocked ? LOCK_ICON_PATH : UNLOCK_ICON_PATH}
                fill={item.isLocked ? "#ef4444" : "#94a3b8"} 
                scaleX={0.7}
                scaleY={0.7}
                x={4}
                y={2}
        ...
        <Layer name="draw-layer">
          {combinedShapes.map((item: any) => (
             <ShapeRenderer 
                ...
                commonProps={{
                    id: item.id,
                    draggable: (activeTool === 'select' || activeTool === null) && !isSpacePressed && !item.isLocked, 
          ...
          {(() => {
            ...
            const shouldHideAnchors = selectedNode && (
